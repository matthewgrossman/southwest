...
class FlightInfo(NamedTuple):
    ...
    def from_flight_info_dict(cls, flight_info_dict: Dict[str, Any]) -> "FlightInfo":
        departure_dt = datetime.datetime.fromisoformat(
            flight_info_dict["departureDateTime"]
        )
        arrival_dt = datetime.datetime.fromisoformat(
            flight_info_dict["arrivalDateTime"]
        )
        return cls(
...
class CheckedInFlightInfo(NamedTuple):
    ...
    @classmethod
    def from_checked_in_flight_dict(
        cls, checked_in_flight_dict: Dict[str, Any]
    ) -> "CheckedInFlightInfo":
        flight = next(
            iter(checked_in_flight_dict["checkInConfirmationPage"]["flights"])
        )
        passenger = next(iter(flight["passengers"]))
        seat = passenger["boardingGroup"] + passenger["boardingPosition"]
        return cls(seat=seat, has_precheck=passenger["hasPrecheck"])
...
class SouthwestAPI:
    ...
    def __init__(self, confirmation_num: str, first_name: str, last_name: str) -> None:
        ...
        self._url_args = (
            f"/{confirmation_num}?first-name={first_name}&last-name={last_name}"
        )
    ...
    def get_reservation_info(self) -> List[FlightInfo]:
        ...
        if not resp.ok:
            if data.get("messageKey") in [Errors.RECORD_NOT_FOUND]:
                raise Exception(
                    "Could not find flight information, please confirm name and confirmation number"
                )
            raise Exception(f"Unknown error: {data}")

        flight_info_dicts = data["viewReservationViewPage"]["shareDetails"][
            "flightInfo"
        ]
    ...
    def checkin(self) -> None:
        ...
        data = resp.json()
        if not resp.ok:
            if data.get("messageKey") in [
                Errors.BEFORE_CHECKIN,
                Errors.AIR_TRAVEL_NOT_OPEN,
            ]:
        ...
        checked_in_flight_info = CheckedInFlightInfo.from_checked_in_flight_dict(
            resp.json()
        )
...
if __name__ == "__main__":
    config = {"confirmation_num": "TKVCKG", "first_name": "John", "last_name": "Smith"}
